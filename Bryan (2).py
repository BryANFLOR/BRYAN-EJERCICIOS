...
l = ("Hola mundo", "" , "Verdadero", "1", "c", "256", "8>19" )
print(l)

#Ejercicio2

rspta = 2 *(4 - 10 + 8)/2* 36 *(1/2)
print(rspta)
print(type(rspta))
...
if ((k**2)-4*j*ñ) < 0:
    ...
    j1 = (-k+sqrt(k**2-(4*j*ñ)))/(2*j)
    j2 = (-k-sqrt(k**2-(4*j*ñ)))/(2*j)
    print("Soluciones de la ecuación:")
    print(j1)
    print(j2)

print("Soluciones: ", j1 , j2)

#Ejercicio6
import math

cate1 = float(input("Ingrese cateto 1: "))
cate2 = float(input("Ingrese cateto 2: "))
...
for pos, dgt_string in enumerate(v_b[::-1]):
	n_d += int(dgt_string) * 2 ** pos

print(f'El número decimal es {n_d}')
...
lbs = float(input("Ingrese su peso en Libras: "))
...
import math
numer = int(input("Ingrese un número entero: "))
if numer > 0:
    digi = int(math.log10(numer))+1
    print(digi)
elif numer == 0:
    digi = 1
    print(digi)
